...
Router.get("/Student", async (req, res) => {
  ...
  Student.find({}).then(result => res.send(result)).catch(err => res.json({
    errMasg: err
  }));
});
...
Router.get("/Teacher", async (req, res) => {
  Teacher.find({}).then(result => res.send(result)).catch(err => res.json({
    errMasg: err
  }));
});
...
module.exports = Router; // 3. تصدير الوحدة | export the module
